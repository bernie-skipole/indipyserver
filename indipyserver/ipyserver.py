...
class IPyServer:
        ...
        try:
            while not self._stop:
                ...
                for driver in self.drivers:
                    ...
                    self._tg.create_task( driver._commsobj.driver_rx(con_id, xmldata) )
                for exdriver in self.exdrivers:
                    # send data to the external drivers
                    self._tg.create_task( exdriver._commsobj.driver_rx(con_id, xmldata) )
                for remcon in self.remotes:
                    self._tg.create_task( remcon._commsobj.driver_rx(con_id, xmldata) )
                for clientconnection in self.connectionpool:
                    # send data out to clients
                    self._tg.create_task( clientconnection._client_tx(con_id, xmldata) )
        finally:
            self.shutdown()
    ...
    def _send_keepalive(self):
        ...
        for remcon in self.remotes:
            ...
            self._tg.create_task( remcon._commsobj.driver_rx(0, xmldata) )
        for clientconnection in self.connectionpool:
            # send data out to clients
            self._tg.create_task( clientconnection._client_tx(0, xmldata) )
    ...
    async def driver_rx(self, con_id, xmldata):
        ...
        if xmldata.tag in DEFTAGS:
            ...
            if devicename in self.driver:
                ...
                raise RuntimeError(f"Device name {devicename} is duplicated in the attached drivers.")
